...
By the end, you will have a Flue agent running on Cloudflare Workers, and you will know how to add reusable agent definitions, R2-backed context, Cloudflare sandboxes, and Durable Object-backed sessions.
...
const triager = defineAgent({
  instructions: 'Search the knowledge base thoroughly before responding. Always cite the specific articles you referenced. Be empathetic but concise.',
});

const agent = await init({ inherit: triager });
